...
def test_models():
    ...
    # Simple site names should be present
    for core in ["deepseek", "chatgpt", "gemini", "kimi", "glm", "qwen", "meta", "arena", "auto"]:
        assert core in ids, f"Missing core model {core}"
    print(f"✓ /v1/models ok ({len(data['data'])} models) - simple site names")

def test_models_all():
    r = client.get("/v1/models/all")
    assert r.status_code == 200
    data = r.json()
    assert len(data["data"]) >= 14
    ids = [m["id"] for m in data["data"]]
    for core in ["deepseek-chat", "gpt-4o", "gemini-2.0-flash", "kimi-k2", "glm-4", "qwen-turbo", "llama-3", "auto"]:
        assert core in ids, f"Missing core model {core} in all"
    print(f"✓ /v1/models/all ok ({len(data['data'])} models)")
