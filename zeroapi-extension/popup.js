// ZeroAPI Popup v2.1 - API-only, no Roblox remnants, with "Use this chat" and ZeroScript compatibility
...
function renderStatus(s) {
  ...
  if (s.apiConnected) {
    ...
    apiState.innerHTML = `✅ <b>API Server</b>: Connected <span class="badge ok">online</span> <span style="opacity:0.6">:${8000}</span>`;
  } else {
  ...
  if (activeTabEl) {
    ...
    if (active) {
      ...
      activeTabEl.innerHTML = `Active chat: <b>${prov.name}</b> <span class="badge ok">${active.provider || prov.id}</span><br><small style="opacity:0.6">${(active.url || "").slice(0,50)}</small>`;
    } else {
